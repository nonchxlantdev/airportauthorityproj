...
import {
  ...
  staffPositionsTableConfigured
} from './utils/workspaceData.js';
import { loadWorkspaceSnapshot } from './utils/offlineDataCache.js';
import './styles.css';

initializeNonCapablePlatformBehavior();
...
function getUserCapabilities(user) {
  ...
  return {
    ...
    canApproveJobs: ['System Administrator', 'General Manager / Senior Management', 'Department Manager'].includes(role)
  };
}
...
async function uploadJobAttachmentsResilient(jobId, files = [], context = {}) {
  try {
    return await uploadJobAttachments(jobId, files);
  } catch (error) {
    if (isOfflineCapable() && isOfflineError(error) && context.user && context.job) {
      await queuePhotoUploads({
        user: context.user,
        job: context.job,
        jobId,
        files
      ...
      return Array.from(files)
        ...
        .map((file) => buildQueuedAttachment(file));
    }
    throw error;
  }
}
...
function DashboardApp({ currentUser, jobs, setJobs, users, setUsers, teams, setTeams, staffPositions, recentActivity, onLogout, onRefreshData }) {
  ...
  const pendingApprovalsCount = useMemo(() => {
    if (!capabilities.canApproveJobs) return 0;
    return visibleJobs.filter((job) => job.approvalStatus === 'Pending Approval').length;
  }, [visibleJobs, capabilities.canApproveJobs]);
  const jobsForListView = showPendingApprovalsOnly
    ? visibleJobs.filter((job) => job.approvalStatus === 'Pending Approval')
    : visibleJobs;

  function handleExportReport(exportJobs) {
    if (!exportJobs.length) {
      toast.info('No jobs to export for the current view.');
      return;
    }
    exportJobsToCsv(exportJobs);
    toast.success(`Exported ${exportJobs.length} job${exportJobs.length === 1 ? '' : 's'} to CSV.`);
  }

  function handleNotificationsClick() {
    if (!pendingApprovalsCount) {
      toast.info('No jobs are waiting for approval.');
      return;
    }
    setShowPendingApprovalsOnly(true);
    setActiveView('jobs');
    toast.info(`${pendingApprovalsCount} job${pendingApprovalsCount === 1 ? '' : 's'} pending approval.`);
  }
  ...
  async function handleUpdateJob(jobId, formData, attachmentFiles = [], existingAttachments = []) {
    ...
    try {
      guardApprovalEdit({
        user: currentUser,
        previousApprovalStatus: existingJob?.approvalStatus,
        nextApprovalStatus: formData.approvalStatus
      });
    } catch (error) {
      toast.error(getOfflineUserMessage(error));
      return false;
    }
    ...
    try {
      uploadedAttachments = await uploadJobAttachmentsResilient(jobId, attachmentFiles, {
        user: currentUser,
        job: existingJob
      });
    } catch (error) {
    ...
    if (error) {
      const hasQueuedPhotos = uploadedAttachments.some((attachment) => attachment.pendingSync);
      if (isOfflineCapable() && isOfflineError(error) && hasQueuedPhotos) {
        ...
        toast.info('Photos saved on this device for sync. Other job field changes still require connectivity.');
        return true;
      }
  ...
  async function handleUpdateJobStatus(jobId, nextStatus, note) {
    ...
    const historyTitle = `Status updated to ${nextStatus}`;
    const historyNote = note || 'Status changed from task detail view.';

    function applyLocalStatusUpdate() {
      setJobs((currentJobs) => currentJobs.map((entry) => {
        ...
        const updatedJob = {
          ...
          status: nextStatus,
          completionDate,
          approvalStatus: nextStatus === 'Completed' ? 'Pending Approval' : entry.approvalStatus,
    ...
    try {
      const { queued } = await runOfflineCapableWrite({
        canQueue: canQueueJobStatusUpdate(currentUser, job, currentUserTeamIds),
        execute: async () => {
          const { error } = await supabase
            .from('jobs')
      ...
      if (queued) {
        toast.info('Status saved on this device and will sync when you are back online.');
      } else {
    ...
    <div className={isSidebarCollapsed ? 'app-shell sidebar-collapsed' : 'app-shell'}>
      ...
      <main className="main-content">
        <TopBar
          ...
          showOfflineMapButton={isOfflineCapable()}
        ...
        <section className="content-pad">
          {activeView === 'dashboard' && (
            <DashboardView
              ...
              onExportReport={() => handleExportReport(dashboardJobs)}
            />
          )}
          {activeView === 'my-tasks' && (
            <MyTasksView
          ...
          {activeView === 'jobs' && (
            <JobsView
              jobs={jobsForListView}
              onCreateJob={() => {
                setShowPendingApprovalsOnly(false);
              ...
              dateFilter={dateFilter}
              setDateFilter={setDateFilter}
              onExportReport={() => handleExportReport(jobsForListView)}
              showPendingApprovalsOnly={showPendingApprovalsOnly}
              onClearPendingFilter={() => setShowPendingApprovalsOnly(false)}
            />
          )}
...
function TopBar({ activeView, currentUser, onToggleSidebar, showOfflineMapButton = false, pendingApprovalsCount = 0, onNotificationsClick }) {
  ...
  return (
    <header className="topbar">
      ...
      <div className="user-tools">
        <PendingSyncBadge />
        {showOfflineMapButton && <OfflineMapDownloadButton />}
        <button
          className={pendingApprovalsCount ? 'notification-button has-alerts' : 'notification-button'}
          type="button"
          aria-label={pendingApprovalsCount ? `${pendingApprovalsCount} jobs pending approval` : 'Notifications'}
          onClick={onNotificationsClick}
        >
          <Bell size={21} />
          {pendingApprovalsCount > 0 && <span>{pendingApprovalsCount}</span>}
        </button>
...
function DashboardView({ dashboardData, jobs, allJobs, recentActivity, onCreateJob, onViewJob, searchTerm, setSearchTerm, canCreateJobs, dateFilter, setDateFilter, selectedMetricFilter, onSelectMetricFilter, onExportReport }) {
  ...
  return (
    <>
      <PageHeading
        ...
        action={<ActionButtons onCreateJob={onCreateJob} canCreateJobs={canCreateJobs} dateFilter={dateFilter} setDateFilter={setDateFilter} onExportReport={onExportReport} />}
      />
      ...
      <section className="dashboard-lower">
        ...
        <aside className="right-rail">
          <RecentActivity activity={recentActivity} allJobs={allJobs} onViewJob={onViewJob} />
...
function JobsView({ jobs, onCreateJob, onViewJob, searchTerm, setSearchTerm, canCreateJobs, dateFilter, setDateFilter, onExportReport, showPendingApprovalsOnly, onClearPendingFilter }) {
  return (
    <>
      <PageHeading
        title={showPendingApprovalsOnly ? 'Jobs Pending Approval' : 'Jobs / Tasks'}
        description={showPendingApprovalsOnly ? 'Completed jobs waiting for manager approval' : 'Create, search, and view airport job records'}
        action={(
          <div className="toolbar-actions">
            {showPendingApprovalsOnly && (
              <button className="secondary-button" type="button" onClick={onClearPendingFilter}>
                Show all jobs
              </button>
            )}
            <ActionButtons onCreateJob={onCreateJob} canCreateJobs={canCreateJobs} dateFilter={dateFilter} setDateFilter={setDateFilter} onExportReport={onExportReport} />
          </div>
        )}
      />
      <RecentJobs
...
function ActionButtons({ onCreateJob, canCreateJobs, dateFilter, setDateFilter, onExportReport }) {
  return (
    <div className="toolbar-actions">
      <DateRangeControl dateFilter={dateFilter} setDateFilter={setDateFilter} />
      <button className="export-button" type="button" onClick={onExportReport}>
        <Download size={18} />
        Export Report
      </button>
...
function JobDetailsDrawer({ job, currentUser, canUpdateStatus, canEditJob, users, teams, onUpdateJob, onUpdateStatus, onClose }) {
  ...
  const approvalLockedOffline = !navigator.onLine;
            ...
            <form className="drawer-edit-form" onSubmit={handleEditSubmit}>
              ...
              <label>
                ...
                <select
                  ...
                  onChange={(event) => updateEditField('approvalStatus', event.target.value)}
                >
                  <option>Not Submitted</option>
                  <option>Pending Approval</option>
                  {!approvalLockedOffline && <option>Approved</option>}
                  {!approvalLockedOffline && <option>Rejected</option>}
                </select>
              </label>
              {approvalLockedOffline && (
                <p className="offline-map-hint">Approve/reject stays online-only so permissions can be verified live.</p>
              )}
              <label>
...
function RecentActivity({ activity, allJobs, onViewJob }) {
  return (
    <section className="panel rail-panel">
      <div className="panel-header">
        <h2><ClipboardList size={17} /> Recent Activity</h2>
      </div>
      <div className="stack-list activity-list">
        {activity.length ? activity.map((item) => (
          <button
            ...
            type="button"
            key={`${item.jobId}-${item.at}-${item.title}`}
            onClick={() => {
          ...
          >
            <div>
              <strong>{item.title}</strong>
              <p>{item.jobId}{item.note ? ` — ${item.note}` : ''}</p>
              <span>{item.at} by {item.by}</span>
            </div>
          </button>
        )) : <EmptyState title="No activity yet" description="Job updates will appear here in real time." compact />}
      </div>
    </section>
  );
}
...
async function loadRecentActivity() {
  const { data, error } = await supabase
    .from('job_history')
    .select('job_id, title, note, actor_name, created_at')
    .order('created_at', { ascending: false })
    .limit(5);
...
function App() {
  ...
  function applyWorkspaceSnapshot(snapshot) {
    ...
    if (snapshot.staffPositions) setStaffPositions(snapshot.staffPositions);
    if (snapshot.sessionUser) setSessionUser(snapshot.sessionUser);
    setIsServingCachedData(true);
  }

  async function refreshData(activeSessionUser = sessionUser) {
    const loaders = [loadProfiles(), loadJobs(), loadTeams(), loadRecentActivity()];
    if (staffPositionsTableConfigured()) {
      loaders.push(loadStaffPositions());
    }
    ...
    try {
      ...
      const [
        ...
        nextTeams,
        nextActivity,
        nextStaffPositions = staffPositions
      ] = results;
      const enrichedJobs = enrichJobsWithTeams(enrichJobsWithUsers(nextJobs, nextUsers), nextTeams);
      ...
      if (staffPositionsTableConfigured()) {
        setStaffPositions(nextStaffPositions);
      }
      ...
      if (isOfflineCapable()) {
        await persistWorkspaceSnapshot({
          ...
          users: nextUsers,
          teams: nextTeams,
          staffPositions: staffPositionsTableConfigured() ? nextStaffPositions : []
        });
      }
    } catch (error) {
      if (isOfflineCapable() && isOfflineError(error)) {
        const restored = await hydrateWorkspaceFromCache(applyWorkspaceSnapshot);
        if (restored) return;
      }
      throw error;
    }
  ...
  useEffect(() => {
    if (!sessionUser || !isOfflineCapable()) return undefined;

    return setupOfflineSyncListeners(() => {
  ...
  useEffect(() => {
    ...
    async function restoreSession() {
      ...
      try {
        if (isOfflineCapable()) {
          cachedSnapshot = await loadWorkspaceSnapshot();
          if (cachedSnapshot) {
        ...
        if (!data.session) {
          if (!cachedSnapshot?.sessionUser && isMounted) {
            setSessionUser(null);
          }
          return;
        }
        ...
        try {
          ...
          if (isMounted && cachedSnapshot?.sessionUser && isOfflineError(error)) {
            setSessionUser(cachedSnapshot.sessionUser);
            return;
          }
          throw error;
        }
        ...
        if (isMounted) {
          if (isOfflineCapable() && isOfflineError(error) && cachedSnapshot) {
            return;
          }
  ...
  return (
    <>
      <OfflinePlatformNotice />
      {sessionUser
    ? (
      <>
        {isServingCachedData && isOfflineCapable() && (
          <div className="offline-data-banner" role="status">
            Showing last saved workspace data while offline.
          </div>
        )}
        <DashboardApp
